...
# --- CARGA DE DATOS SIN ESTRUCTURAS COMPLEJAS ---
try:
df_agentes = pd.read_csv(f"{SHEET_BASE_URL}agentes").dropna(how='all')
df_agentes.columns = [str(c).strip().lower() for c in df_agentes.columns]
except:
df_agentes = pd.DataFrame()

try:
df_sorteos = pd.read_csv(f"{SHEET_BASE_URL}sorteos").dropna(how='all')
df_sorteos.columns = [str(c).strip().lower() for c in df_sorteos.columns]
except:
df_sorteos = pd.DataFrame()

try:
df_entregas_base = pd.read_csv(f"{SHEET_BASE_URL}entregas").dropna(how='all')
df_entregas_base.columns = [str(c).strip().lower() for c in df_entregas_base.columns]
except:
df_entregas_base = pd.DataFrame()
